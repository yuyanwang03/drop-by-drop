...
with avg_pernoctation_col:
    avg_pernoctation = st.number_input("Introdueix l'estada mitjana dels turistes (en dies)",value=1.0, min_value=1.0, max_value=90.0, step=0.1)
...
if st.button('Calcular Distribució'):
    ...
    if concentration_option == 'Concentrar turistes':
        if total_tourists < sum([period['tourists_concentrated'] for period in st.session_state['concentration_periods']]):
            ...
            tourist_distribution = calculate_distribution(
                num_days=(end_day_num - start_day_num + 1),
                total_tourists=total_tourists,
                start_date=start_date,
                concentration_periods=[(period['start_concentration_day'], period['end_concentration_day'], period['tourists_concentrated']) for period in st.session_state['concentration_periods']]
            )
            st.write(f"Total initial tourists: {total_tourists}")
            # Prepare the DataFrame for display
            df = pd.DataFrame({
                'start_date': [start_date + timedelta(days=i) for i in range(end_day_num)],
                'tourist': tourist_distribution
            })
            # Prepare end_date column for smooth distribution
            df = df[['start_date', 'tourist']]
    else:
        ...
        # Prepare end_date column for smooth distribution
        #df['end_date'] = df['start_date'].shift(-1, fill_value=df['start_date'].max())
        df = df[['start_date', 'tourist']]
